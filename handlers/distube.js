...
//      if (queue.prev) return
      ...
      {
        setTimeout(function(){ 
          m.delete().catch(err => console.error());
          ...
          case "skipbut_but":
            {
              ...
              if (queue.autoplay || queue.songs.length > 1){
                if (!queue.paused) {
                  await m.edit({ embeds: [new Discord.MessageEmbed().setTitle(`<:prev:987978423240654889> | Anterior \n\`${trimmedString} - ${song.formattedDuration}\``).setColor("#b362ef")],components: []});
                  queue.skip(message)}
                  return;
                }
                if (queue.paused) {
          ...
          case "previous_but":
            {
              ...
              await queue.previous();
                if (!queue.paused) {
                  await m.edit({ embeds: [new Discord.MessageEmbed().setTitle(`<:next:987982303118639124> | Siguiente \n\`${trimmedString} - ${song.formattedDuration}\``).setColor("#b362ef")],components: []})
                  //queue.skip(message)
                }
                if (queue.paused) {
                  await m.edit({ embeds: [new Discord.MessageEmbed().setTitle(`<:viniloMUSICA:988191479770005585> | Pausado...`).setThumbnail('https://i.imgur.com/fSwwvs8.png')
    ...
    setInterval(() => {
      ...
      if(queue.playing){m.edit({
        embeds: [new Discord.MessageEmbed().setTitle(`<:viniloMUSICA:988191479770005585> | Reproduciendo...`).setThumbnail('https://i.imgur.com/fSwwvs8.png')
        .setDescription(`[${trimmedString}](${song.url})\n${queue.formattedCurrentTime}⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀ ${song.formattedDuration}\n${filledBar(track.duration, queue.currentTime, 17, '<:cuadradosilver:990277877276749904>', '<:128cuadwindem3:990299473970343977>')[0]}\n \ \ \ \  ${status(queue)}`).setColor("#b362ef")],
         components: [botones, botones2]
         }).catch(err => console.error());} 
           }, 5*1000 /* once per minute */)
          return;
